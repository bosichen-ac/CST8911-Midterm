...
module.exports = async function (context, req) {
  try {
    ...
    if (typeof body !== "object") {
      context.res = { status: 400, body: { error: "Body must be JSON." } };
      return;
    }
    if (!body.id) {
      context.res = { status: 400, body: { error: "Property 'id' is required." } };
      return;
    }

    const container = await getCosmosContainer(context);
    const { resource } = await container.items.upsert(body); // partitionKey '/id'
    context.res = { status: 200, body: resource };
  } catch (err) {
    context.log.error("UpsertItem failed:", err);
    context.res = { status: 500, body: { error: "Server error upserting item." } };
  }
}